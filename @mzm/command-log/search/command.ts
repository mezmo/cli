...
const search = new MZMCommand()
  ...
  .action(async (options: any, query?: string) => {
    ...
    if (params.tag) params.tag = toArray(options.tag).join(',')
    if (params.level) params.level = toArray(options.level).join(',')
    if (params.app) params.app = toArray(options.app).join(',')
